...
bool Camera::isCulling2D(const Vec3 & position, const Vec3 & size)
{
	...
	return !(MathUtility::isInRange(position.x, -halfWidth - halfSize, halfWidth + halfSize) &&	//横方向の判定
		MathUtility::isInRange(position.y, -halfHeight - halfSize, halfHeight + halfSize));		//縦方向の判定
}
