...
bool Camera::isCulling2D(const Vec3 & position, const Vec3 & size)
{
	...
	return !(MathUtility::isInRange(position.x, -halfWidth - halfSizeX, halfWidth + halfSizeX) &&	//横方向の判定
		MathUtility::isInRange(position.y, -halfHeight - halfSizeY, halfHeight + halfSizeY));		//縦方向の判定
}
